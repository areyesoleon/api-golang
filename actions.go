...
func (this *Message) setMensaje(data string) {
	this.Mensaje = data
}
...
func MovieRemove(w http.ResponseWriter, r *http.Request) {
	...
	//results := Message{"success", "La pelicula con ID " + movieId + " Fue eliminada"}

	message := new(Message)
	message.setStatus("success")
	message.setMensaje("La pelicula con ID " + movieId + " Fue eliminada")
	results := message
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
	json.NewEncoder(w).Encode(results)
